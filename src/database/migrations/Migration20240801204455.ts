...
export class Migration20240801204455 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table `cart` (`id` text not null, `user_id` text not null, `is_deleted` integer not null, primary key (`id`));');
    ...
    this.addSql('create table `cart_item` (`id` text not null, `product_id` text not null, `cart_id` text not null, `order_id` text not null, `count` integer not null, constraint `cart_item_product_id_foreign` foreign key(`product_id`) references `product`(`id`) on update cascade, constraint `cart_item_cart_id_foreign` foreign key(`cart_id`) references `cart`(`id`) on update cascade, constraint `cart_item_order_id_foreign` foreign key(`order_id`) references `order`(`id`) on update cascade, primary key (`id`, `product_id`));');
